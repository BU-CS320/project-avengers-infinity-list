...
instance Functor (EnvUnsafe e) where
  ...
  fmap f origEnv = EnvUnsafe $ (\ env ->
    case (runEnvUnsafe origEnv env) of (Ok v) -> Ok $ f v
                                       (Error msg) -> Error msg)
...
instance Monad (EnvUnsafe e) where
  ...
  return val = EnvUnsafe $ \ env -> Ok val

  --(>>=) :: EnvUnsafe a -> (a -> EnvUnsafe b) -> EnvUnsafe b
  origEnv >>= f = EnvUnsafe $ (\ env ->
    case (runEnvUnsafe origEnv env) of (Ok val) -> (runEnvUnsafe (f val)  env)
                                       (Error msg) -> Error msg)
